...
class Crud {
  ...
  postFileAndDataSignUp(String linkUrl, Map data, Map<String, String> headers,
      File file, File idFile) async {
    var request = http.MultipartRequest('Post', Uri.parse(linkUrl));
    int fileLength = await file.length();
    ...
    var multiFile = http.MultipartFile('proofPhoto', streamData, fileLength,
        filename: basename(file.path));
    var idMultiFile = http.MultipartFile('idPhoto', idStreamData, idFileLength,
        filename: basename(idFile.path));
    request.files.add(multiFile);
